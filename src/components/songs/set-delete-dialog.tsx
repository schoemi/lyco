...
interface SetDeleteDialogProps {
  open: boolean;
...
export default function SetDeleteDialog({ open, set, onClose, onDeleted }: SetDeleteDialogProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="mb-2 text-lg font-semibold text-neutral-900">Set löschen</h2>
        <p className="mb-1 text-sm text-neutral-600">
          Möchten Sie das Set <span className="font-medium">&quot;{set.name}&quot;</span> wirklich löschen?
        </p>
        <p className="mb-4 text-sm text-error-600">
          Alle Song-Zuordnungen werden entfernt, die Songs bleiben erhalten.
        </p>
        {error && (
