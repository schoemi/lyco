...
export default function RueckwaertsPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // Track session with RUECKWAERTS
        try {
  ...
  // --- Derived values (DESCENDING sort by orderIndex) ---
  ...
  const handleWeiter = useCallback(() => {
    ...
    if (istLetzteZeile) {
      ...
      if (allCompleted && !allCompleteFired.current) {
        ...
        // Create completion session with RUECKWAERTS
        fetch("/api/sessions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ songId: id, lernmethode: "RUECKWAERTS" }),
